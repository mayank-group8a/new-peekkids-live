...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  itemHtml = itemHtml.replace(/{{itemQuickViewButton}}/g, buildQuickViewButton(data));

  
  // Add main attribute (Always put at the end of this function)
  // console.log('data: ',data);
  itemHtml = itemHtml.replace(/{{itemSku}}/g, data.sku);
...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    html = '<p class="bb boost-pfs-filter-product-item-price">';
    if (onSale) {
...
// Build Quick view Button
function buildQuickViewButton(data) {
  return `<div class="quick-view-button"><button type="button" class="quick-view" data-handle="${data.handle}">Quick Look</button></div><script class="product-script" data-id="product-json-${data.id}" type="application/json" data-uw-styling-context="true">${JSON.stringify(data)}</script>`;
}
// Build Quick view Button
function updatePromotion() {
  var promoRow = '';
  var promoIteamLength = '';
  var promoIteam = '';

  var promoRow = document.querySelector('.promo-row');
  var promoIteamLength = promoRow.querySelectorAll('.promo-iteam').length;
  if(promoIteamLength > 0 && $('.boost-pfs-filter-products').find('.promo-iteam').length===0){
    var promoIteam = promoRow.querySelector('.promo-iteam');
    // console.log(promoIteam);
    $($(promoIteam).clone()).prependTo('.boost-pfs-filter-products')
  }
}
...
function buildReview(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_product_review') &&
    ...
    html = '<span class="stamped-product-reviews-badge" data-id="{{itemId}}"data-product-sku="{{itemHandle}}" data-product-type="{{itemType}}" data-product-title="{{itemTitle}}" style="display:block;">{{product.metafields.stamped.badge}}</span>';  }
  return html;
}
